...
app.use(cors({
	origin: ['http://10.0.2.2:3000', 'http://localhost:3000'], //allow requests from web and Android Studios emulator
...
app.post("/api/signup", async (req, res) => {
	...
	try {
	  ...
	  db.query("SELECT * FROM Account WHERE Email = ?", [email], async (err, results) => {
		...
		db.query(
		  "INSERT INTO Account (Email, Pass, AccountType) VALUES (?, ?, ?)", [email, hashedPassword, role], (err, result) => {
			if (err) {
